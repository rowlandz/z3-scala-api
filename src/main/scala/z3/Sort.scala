...
sealed abstract class Sort private[z3] (private[z3] val ptr: Long) {
  lazy val name: String = Native.getSymbolString(gcptr, Native.getSortName(gcptr, ptr))

  def equalsSort(other: Sort): Boolean = Native.isEqSort(gcptr, ptr, other.ptr)

  override def toString: String = Native.sortToString(gcptr, ptr)
}

/** The sort that contains the boolean values `true` and `false`. The sort itself can be
 *  constructed with [[z3.boolSort]] and its values with [[z3.bool]]. */
final class BoolSort private[z3] (ptr: Long) extends Sort(ptr)

/** The sort that contains the integers.
 *  The sort itself can be constructed with [[z3.intSort]] and its values with `z3.int`. */
final class IntSort private[z3] (ptr: Long) extends Sort(ptr)

/** The sort that contains the real numbers.
 *  The sort itself can be constructed with [[z3.realSort]] and its values with `z3.real`. */
final class RealSort private[z3] (ptr: Long) extends Sort(ptr)
...
final class BitVectorSort private[z3] (ptr: Long) extends Sort(ptr) {
  lazy val size: Int = Native.getBvSortSize(gcptr, ptr)
}
 ...
 *  }}}
 */
class DatatypeSort private[z3] (ptr: Long) extends Sort(ptr) {

  /** List of constructors for this data type. The order of the constructors is the same as in the type definition. */
  lazy val constructors: List[FuncDecl] = (0 until Native.getDatatypeSortNumConstructors(gcptr, ptr)).map { i =>
    new FuncDecl(Native.getDatatypeSortConstructor(gcptr, ptr, i))
  }.toList

  /** List of recognizers for this data type. The order of the recognizers is the same as in the type definition. */
  lazy val recognizers: List[FuncDecl] = (0 until Native.getDatatypeSortNumConstructors(gcptr, ptr)).map { i =>
    new FuncDecl(Native.getDatatypeSortRecognizer(gcptr, ptr, i))
  }.toList

  /** Returns the constructor for this data type with name `name`. */
  def getConstructor(name: String): FuncDecl = GlobalState.datatypeConstructorNames(this.name).indexOf(name) match {
    case i if i >= 0 => new FuncDecl(Native.getDatatypeSortConstructor(gcptr, ptr, i))
  ...
  def getRecognizer(name: String): FuncDecl = GlobalState.datatypeRecognizerNames(this.name).indexOf(name) match {
    case i if i >= 0 => new FuncDecl(Native.getDatatypeSortRecognizer(gcptr, ptr, i))
  ...
  def getAccessor(constructor: String, field: String): FuncDecl = {
    val constructorIndex = GlobalState.datatypeConstructorNames(this.name).indexOf(constructor)
    val accessorIndex = GlobalState.datatypeFields(this.name)(constructorIndex).indexWhere(_.name == field)
    new FuncDecl(Native.getDatatypeSortConstructorAccessor(gcptr, ptr, constructorIndex, accessorIndex))
  }

  /** Returns the SMTLib2  `(declare-datatype ...)` type declaration string. */
  def smt2string: String = {
    val smt2constructors = GlobalState.datatypeConstructorNames(name).zip(GlobalState.datatypeFields(name)).map {
      case (c, fs) =>
 ...
 *  }}}
 */
class StructSort private[z3] (ptr: Long) extends DatatypeSort(ptr) {
  /** List of field projector functions for this struct type. A projector function extracts a field from a struct. */
  lazy val projectors: List[FuncDecl] = GlobalState.datatypeFields(this.name)(0).indices.map { i =>
    new FuncDecl(Native.getDatatypeSortConstructorAccessor(gcptr, ptr, 0, i))
  }.toList

  /** Returns the projector function for `field`. */
  def getProjector(field: String): FuncDecl = {
    val accessorIndex = GlobalState.datatypeFields(this.name)(0).indexWhere(_.name == field)
    new FuncDecl(Native.getDatatypeSortConstructorAccessor(gcptr, ptr, 0, accessorIndex))
  }
}


class ArraySort private[z3] (ptr: Long) extends Sort(ptr)

// TODO support this
class SetSort private[z3] (ptr: Long) extends ArraySort(ptr)
...
class UninterpretedSort private[z3] (ptr: Long) extends Sort(ptr)

private[z3] object Sort {
  def create(ptr: Long): Sort = Z3_sort_kind.fromInt(Native.getSortKind(gcptr, ptr)) match {
    case Z3_sort_kind.Z3_UNINTERPRETED_SORT => new UninterpretedSort(ptr)
    case Z3_sort_kind.Z3_BOOL_SORT => new BoolSort(ptr)
    case Z3_sort_kind.Z3_INT_SORT => new IntSort(ptr)
    case Z3_sort_kind.Z3_REAL_SORT => new RealSort(ptr)
    case Z3_sort_kind.Z3_BV_SORT => new BitVectorSort(ptr)
    case Z3_sort_kind.Z3_ARRAY_SORT => new ArraySort(ptr)
    case Z3_sort_kind.Z3_DATATYPE_SORT => new DatatypeSort(ptr)
